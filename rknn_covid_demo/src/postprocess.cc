  ...
  for (int a = 0; a < 3; a++) {
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        int8_t box_confidence = input[a * grid_len * PROP_BOX_SIZE + i * grid_w * PROP_BOX_SIZE + j * PROP_BOX_SIZE + 4];
        if (box_confidence >= thres_i8) {
          int     offset = a * grid_len * PROP_BOX_SIZE + i * grid_w * PROP_BOX_SIZE + j * PROP_BOX_SIZE;
